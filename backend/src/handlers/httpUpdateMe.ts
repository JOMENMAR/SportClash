...
const inner: AsyncHttpHandler = async (event) => {
  ...
  if (hasOwn(body, "nombre"))
    set.nombre = optStringAllowEmpty(body.nombre, "nombre", 60);
  if (hasOwn(body, "apodo"))
    set.apodo = optStringAllowEmpty(body.apodo, "apodo", 40);
  ...
  if (hasOwn(body, "profileIconKey"))
    set.profileIconKey = optStringAllowEmpty(
      body.profileIconKey,
      "profileIconKey",
      40,
    );
  if (hasOwn(body, "profileBanner"))
    set.profileBanner = optEnum(
      body.profileBanner,
      PROFILE_BANNER_KEYS,
      "profileBanner",
    );
  if (hasOwn(body, "profileAccent"))
    set.profileAccent = optEnum(
      body.profileAccent,
      PROFILE_ACCENT_KEYS,
      "profileAccent",
    );
  if (hasOwn(body, "profileAccentHex"))
    set.profileAccentHex = optHexAllowEmpty(
      body.profileAccentHex,
      "profileAccentHex",
    );

  if (hasOwn(body, "profilePageBg"))
    set.profilePageBg = optEnum(
      body.profilePageBg,
      PROFILE_PAGE_BG_KEYS,
      "profilePageBg",
    );
  if (hasOwn(body, "profilePageBgHex"))
    set.profilePageBgHex = optHexAllowEmpty(
      body.profilePageBgHex,
      "profilePageBgHex",
    );

  if (hasOwn(body, "status"))
    set.status = optTrimmedString(body.status, "status", 40);
